...
class Translator
{
    ...
    protected function execute($uri, array $parameters)
    {
        ...
        if ($remoteResult === false) {
            throw new TException(curl_error($this->handler), curl_errno($this->handler));
        }
        ...
        if (!$result) {
            ...
            throw new TException(sprintf('%s: %s', self::MESSAGE_INVALID_RESPONSE, $errorMessage));
        } elseif (isset($result['code']) && $result['code'] > 200) {
            throw new TException($result['message'], $result['code']);
        }
